defmodule ScalarPlugTest do
  ...
  describe "call/2" do
    test "it returns the correct response when the options are passed as a mfa", %{
      conn: conn,
      url: url,
      spec_href: spec_href
    } do
      ...
      opts =
        ScalarPlug.init({ScalarPlugTest, :opts})
      ...
      assert Floki.find(html, "body > script[id='api-reference'][data-url='#{spec_href}']")
             |> List.first() != nil
    ...
    test "it returns the correct response when the options are passed as a function", %{
      conn: conn,
      url: url,
      image_url: image_url,
      x_handle: x_handle,
    ...
    test "it returns the correct response when the options are passed as a keyword list", %{
      conn: conn,
      url: url,
      x_handle: x_handle,
      spec_href: spec_href,
  ...
  def opts do
    [path: "/api/docs", spec_href: "/api/spec"]
  end
end
